...
public:
  ...
  LinkedList(std::initializer_list<T> il) : head{nullptr}, m_size{il.size()} {
    if (il.size() == 0)
      return;
  ...
  bool insert(const std::size_t pos, const T &value) {
    if (pos > m_size)
      return false;
    m_size++;
  ...
  _Node *get(const std::size_t pos) {
    if (pos > m_size)
      return nullptr;
    _Node *temp = head;
  ...
  bool push_front(const T &value) { return insert(0, value); }
